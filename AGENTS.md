...
- **Backend orchestration**: `TripAgentServiceImpl` supplies the authoritative `TripState` and supported field metadata, validates and normalizes model output, persists plans, and never delegates data integrity or authorization to either Agent. `TripItineraryAssembler` deterministically queries travel providers and builds the itinerary skeleton after the PLAN Agent returns a valid macro route.
