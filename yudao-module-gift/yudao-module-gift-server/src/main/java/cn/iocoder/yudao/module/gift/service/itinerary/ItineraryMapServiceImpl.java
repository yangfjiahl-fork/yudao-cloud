...
public class ItineraryMapServiceImpl implements ItineraryMapService {
    ...
    private List<ExploreItem> queryExploreItems(String city, String keyword, String category, int pageNo, int pageSize,
        ...
        for (AmapPoiTypeEnum poiType : poiTypes) {
            try {
                switch (poiType) {
                    case SCENIC -> itineraryTravelQueryService.queryScenicSpots(city, keyword, pageNo, perCategory).forEach(spot ->
                    ...
                    case FOOD -> itineraryTravelQueryService.queryRestaurants(city, keyword, pageNo, perCategory).forEach(place ->
                    ...
                    case HOTEL -> itineraryTravelQueryService.queryHotels(city, keyword, pageNo, perCategory).forEach(place ->
                            result.add(toExploreItem(poiType, place.poiId(), place.name(), place.longitude(), place.latitude(),
                                    place.address(), place.imageUrl(), place.rating(), latitude, longitude)));
                    case SHOPPING -> itineraryTravelQueryService.queryShopping(city, keyword, pageNo, perCategory).forEach(place ->
                            result.add(toExploreItem(poiType, place.poiId(), place.name(), place.longitude(), place.latitude(),
                                    place.address(), place.imageUrl(), place.rating(), latitude, longitude)));
                }
